#!/bin/bash
# FastAPI Async Performance Testing Script
# Runs Hybrid 0.9 async tests with 100 users (limit 200)
...
echo "╔══════════════════════════════════════════════════════════════════════╗"
echo "║              FASTAPI ASYNC PERFORMANCE TESTS (Hybrid 0.9)            ║"
echo "╚══════════════════════════════════════════════════════════════════════╝"
echo ""
echo "Configuration:"
...
echo "  - Hybrid 0.9 Async tests: ${#USER_COUNTS[@]} user counts × 1 test = ${#USER_COUNTS[@]} tests"
echo "  Total: ${#USER_COUNTS[@]} tests"
echo ""

# Check if we're in the right directory
if [ ! -f "locustfile_hybrid_09_async_fastapi.py" ]; then
...
run_test() {
    ...
    # Create report directory with RF value (async clearly marked)
    REPORT_DIR="../reports/multi_collection/fastapi_async_RF${RF_VALUE}_Users${user_count}_Limit${LIMIT}"
    mkdir -p "$REPORT_DIR"
...
# Run Hybrid 0.9 Async tests for each user count
echo ""
echo "${BLUE}═══════════════════════════════════════════════════════════════════════${NC}"
echo "${BLUE}HYBRID 0.9 ASYNC TESTS${NC}"
echo "${BLUE}═══════════════════════════════════════════════════════════════════════${NC}"

for USER_COUNT in "${USER_COUNTS[@]}"; do
    ((test_counter++))
    run_test "locustfile_hybrid_09_async_fastapi.py" "hybrid_09_async" "$USER_COUNT" "$test_counter" "$TOTAL_TESTS"
done

# Generate combined report for FastAPI async tests
echo ""
echo "═══════════════════════════════════════════════════════════════════════"
echo "Generating Combined FastAPI Async Report"
echo "═══════════════════════════════════════════════════════════════════════"
echo ""
...
python3 generate_fastapi_async_report.py
if [ $? -eq 0 ]; then
    echo ""
    echo "✅ Combined FastAPI async report generated"
else
    echo ""
...
echo "╚══════════════════════════════════════════════════════════════════════╝"
echo ""
echo "📊 Reports saved to: reports/multi_collection/fastapi_async_RF${RF_VALUE}_Users*/"
echo ""
echo "Summary:"
echo "  ✅ Hybrid 0.9 Async: ${#USER_COUNTS[@]} tests (${USER_COUNTS[*]} users)"
...
for USER_COUNT in "${USER_COUNTS[@]}"; do
    ...
    echo "     - Hybrid 0.9 Async: $REPORT_DIR/hybrid_09_async_report.html"
done
echo ""
...
echo "   ../reports/multi_collection/fastapi_async_combined_RF${RF_VALUE}_${USERS_STR}_Limit${LIMIT}.html"
echo ""
echo "To view reports:"
for USER_COUNT in "${USER_COUNTS[@]}"; do
    REPORT_DIR="../reports/multi_collection/fastapi_async_RF${RF_VALUE}_Users${USER_COUNT}_Limit${LIMIT}"
    echo "  open $REPORT_DIR/*_async_report.html"
done
echo "  open ../reports/multi_collection/fastapi_async_combined_RF${RF_VALUE}_${USERS_STR}_Limit${LIMIT}.html"
echo ""
