"""
Generate combined performance test report for FastAPI async tests.
Aggregates results from BM25 async and Hybrid 0.9 async tests across different user counts.
"""
...
def scan_fastapi_async_reports():
    """Scan all fastapi_async_* folders and gather data"""
    ...
    if user_counts_str:
        ...
        pattern = os.path.join(base_dir, f"fastapi_async_RF{rf_value}_Users*_Limit{limit}")
    ...
    # FastAPI async reports folders
    base_dir = '../reports/multi_collection'
    
    # Search types for FastAPI async
    search_types = ['bm25_async', 'hybrid_09_async']
    
    for user_count in sorted(user_counts, key=lambda x: int(x)):
        # FastAPI async folder pattern: fastapi_async_RF{rf}_Users{users}_Limit{limit}
        folder = os.path.join(base_dir, f"fastapi_async_RF{rf_value}_Users{user_count}_Limit{limit}")
...
def generate_html_report(results, rf_value, limit):
    """Generate comprehensive HTML report for FastAPI async tests"""
    
    html = """
<!DOCTYPE html>
<html>
<head>
    <title>FastAPI Async Performance Test - Combined Report</title>
    <style>
        body {
...
<body>
    <div class="header">
        <h1>FastAPI Async Performance Test - Combined Report</h1>
        <div class="subtitle">
            Comprehensive analysis: BM25 Async vs Hybrid 0.9 Async across different user counts<br>
...
"""
    ...
    <div class="section">
        ...
        <div>
            ...
            <div class="summary-box">
                <h3>Search Types</h3>
                <p>BM25 Async, Hybrid 0.9 Async</p>
            </div>
            <div class="summary-box">
            ...
            <div class="summary-box">
                <h3>Architecture</h3>
                <p>FastAPI /graphql/async endpoint<br>Parallel collection queries via asyncio.gather</p>
            </div>
        </div>
    </div>
"""
    ...
    <div class="section">
        ...
        <table>
            <tr>
                <th>User Count</th>
                <th>BM25 Async</th>
                <th>Hybrid 0.9 Async</th>
                <th>Difference</th>
            </tr>
"""
    
    for user_count in sorted(results.keys(), key=lambda x: int(x)):
        bm25_metrics = results[user_count].get('bm25_async', {})
        hybrid_metrics = results[user_count].get('hybrid_09_async', {})
...
"""
        ...
        for search_type, display_name in [
            ('bm25_async', 'BM25 Async'),
            ('hybrid_09_async', 'Hybrid 0.9 Async')
        ]:
...
def main():
    """Main function"""
    print("=" * 70)
    print("FASTAPI ASYNC COMBINED PERFORMANCE REPORT GENERATOR")
    print("=" * 70)
    print("\nScanning FastAPI async reports folders...")
    print("-" * 70)
    
    # Scan all reports
    results = scan_fastapi_async_reports()
    
    if not results:
        print("\n❌ No data found!")
        print("   Make sure you have fastapi_async_* folders with CSV files")
        print("   Expected pattern: fastapi_async_RF{rf}_Users{count}_Limit{limit}/")
        return 1
    ...
    print("\n📝 Generating combined FastAPI async HTML report...")
    ...
    output_file = f"../reports/multi_collection/fastapi_async_combined_RF{rf_value}_{users_str}_Limit{limit}.html"
