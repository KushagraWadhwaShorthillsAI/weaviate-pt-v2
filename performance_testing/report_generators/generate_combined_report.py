...
def scan_reports():
    ...
    for limit in limits:
        # Try new naming pattern: reports_RF{rf}_Users{users}_Limit{limit}
        folder_new = os.path.join(base_dir, f"reports_RF{rf_value}_Users{user_count}_Limit{limit}")
        # Try old naming pattern: reports_{limit}
        folder_old = os.path.join(base_dir, f"reports_{limit}")
        
        # Use new pattern if exists, otherwise fall back to old pattern
...
def main():
    ...
    # Standardized naming: multi_collection_combined_RF{rf}_Users{users}_Limit{limit}.html
    ...
    output_file = f"../reports/multi_collection/multi_collection_combined_RF{rf_value}_Users{user_count}.html"
