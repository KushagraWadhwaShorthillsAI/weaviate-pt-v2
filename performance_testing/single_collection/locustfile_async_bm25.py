"""
Weaviate Async Client - BM25 Search on Single Collection
Uses Weaviate's native Python async client (NO GraphQL strings!)

Usage:
    locust -f locustfile_async_bm25.py --users 100 --spawn-rate 5 --run-time 5m --headless
"""
...
def on_locust_init(environment, **kwargs):
    ...
    print("=" * 70)
    print("Loading BM25 query file for Weaviate Async Client...")
    print("=" * 70)
...
class WeaviateAsyncBM25User(User):
    """User that performs BM25 searches using Weaviate async client"""
    ...
    async def search_bm25_async(self, query_text, limit):
        """Execute BM25 search using Weaviate async client"""
        start_time = time.time()
        
        try:
            collection = self.client.collections.get(config.WEAVIATE_CLASS_NAME)
            
            # BM25 search - NO GraphQL!
    ...
    def search_bm25(self):
        ...
        try:
            ...
            result = loop.run_until_complete(
                self.search_bm25_async(query_text, limit)
            )
