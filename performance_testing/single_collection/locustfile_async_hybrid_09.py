"""
Weaviate Async Client - Hybrid Search (alpha=0.9) on Single Collection
Uses Weaviate's native Python async client (NO GraphQL strings!)

Usage:
    locust -f locustfile_async_hybrid_09.py --users 100 --spawn-rate 5 --run-time 5m --headless
"""
...
def on_locust_init(environment, **kwargs):
    ...
    try:
        ...
        print(f"  Hybrid alpha: 0.9 (10% BM25, 90% vector)")
        print("=" * 70)
...
class WeaviateAsyncHybrid09User(User):
    """User that performs hybrid searches (alpha=0.9) using Weaviate async client"""
    ...
    async def search_hybrid_async(self, query_text, query_vector, limit):
        ...
        try:
            ...
            # Hybrid search with alpha=0.9 - NO GraphQL!
            results = await collection.with_consistency_level(
                consistency_level=ConsistencyLevel.ONE
            ).query.hybrid(
                query=query_text,
                vector=query_vector,
                alpha=0.9,  # 10% BM25, 90% vector
        ...
        try:
            ...
            if result["success"]:
                self.environment.events.request.fire(
                    ...
                    name="Hybrid_09_Single_Collection",
