...
async def graphql_normal(request: GraphQLRequest):
    ...
    This endpoint accepts a GraphQL query (like the one from queries_hybrid_09_200.json)
    that searches all collections in a single request.
    ...
    logger.debug(f"[REQ-{request_id}] Query length: {len(request.query)} characters")
...
async def graphql_async_lookup(request: QueryTextRequest):
    ...
    # Extract vector from GraphQL query if present
    graphql_query = query_data.get("graphql", "")
    query_vector = extract_vector_from_graphql(graphql_query)
    
    # Determine alpha from query_data or detect from query_file/GraphQL query
    # Check if this is a BM25 query by looking at query_file name or GraphQL query structure
    is_bm25_query = False
